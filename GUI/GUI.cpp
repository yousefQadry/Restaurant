...
void GUI::PrintMessage(string msg) const	//Prints a message on status bar
{
	if (Lines == WindHeight)
	{
		...
		pWind->DrawString(10, WindHeight - (int)(StatusBarHeight / 1), msg);
		Lines += 20;

	}
	else if (Lines > WindHeight)
	{
		pWind->SetPen(DARKBLUE);
		pWind->SetFont(18, BOLD, BY_NAME, "Arial");
		pWind->DrawString(10, Lines - (int)(StatusBarHeight / 1), msg);
		Lines += 20;
...
void GUI::RemoveItem(int id) {
	DrawingItem* pDitem;
	for (int i = 0; i < DrawingItemsCount; i++)
	{
		pDitem = DrawingList[i];
		if (pDitem->ID == id && (DrawingList[i]->region == ORD_REG || DrawingList[i]->region == SRV_REG) ) {
			for (int j = i; j < DrawingItemsCount - 1 ; j++)
			{
				DrawingList[j] = DrawingList[j + 1];
			}
				DrawingItemsCount--;
				return;
		}
